...
class EditGraduateScreen extends StatefulWidget {
  final Map<String, dynamic> graduate;

  const EditGraduateScreen({super.key, required this.graduate});

  @override
  _EditGraduateScreenState createState() => _EditGraduateScreenState();
}

class _EditGraduateScreenState extends State<EditGraduateScreen> {
  ...
  void initState() {
    super.initState();
    debugPrint('Graduate data received: ${widget.graduate}');
    _initializeForm();
    _fetchInstrumentsAndSedes();
  }

  void _initializeForm() {
    _firstNameController.text = widget.graduate['first_name'] ?? '';
    _lastNameController.text = widget.graduate['last_name'] ?? '';
    _emailController.text = widget.graduate['email'] ?? '';
    _idNumberController.text = widget.graduate['identification_number'] ?? '';

    final instrumentsList =
        widget.graduate['graduate_instruments'] as List<dynamic>?;
    if (instrumentsList != null && instrumentsList.isNotEmpty) {
      ...
      debugPrint('No graduate_instruments data found');
      _selectedInstrumentIds = [];
    }

    final sedesList = widget.graduate['graduate_sedes'] as List<dynamic>?;
    if (sedesList != null && sedesList.isNotEmpty) {
      ...
      debugPrint('No graduate_sedes data found');
      _selectedSedeIds = [];
    }
  ...
  Future<void> _updateGraduate() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Egresado actualizado con éxito')),
      );
    } catch (e) {
      debugPrint('Error updating graduate: $e');
      String errorMessage = 'Error al actualizar egresado';
                      ...
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            ...
                            onPressed: _isLoading ? null : _updateGraduate,
                          ),
                        ),
